from random import SystemRandom #importando a biblioteca random
import pickle as pck #importando a biblioteca pickle para poder transformar em pickle
import math
import os
random=SystemRandom()
######
...
def modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz,patches_sorteados,bOUm,solt_posi,solt_valor,hec10_bordas,para_lista_inicial,borda_meio,loop,lib_extras):
	qnd_onde_liberou=[]
	...
	for t in range(tempo_final):
		for l in range(lin):
			for c in range(col):
				...
				if t>0:
					tempo_dinamica=dinamica_iterativa(g_all,pop_iniciais,ocupacao_total_tempo,media_regional_tempo,pop_migrante,c,l,t,a1,a2,a3,lambda1,lambda2,TS,TH,H0,h0,f0,ocupacao_total)
					#############
					#migração
					#############
...
def dinamica_iterativa(g_all,pop_iniciais,ocupacao_total_tempo,media_regional_tempo,pop_migrante,c,l,t,a1,a2,a3,lambda1,lambda2,TS,TH,H0,h0,f0,ocupacao_total):
	...
	if g_all[l][c][0][t-1]==0:
		...
		g_all[l][c][0].append(g_all[l][c][2][t-1]*(1-rfP))
		if g_all[l][c][0][t]<=0.01:
			g_all[l][c][0][t]=0
	...
	if g_all[l][c][1][t-1]==0:
		...
		g_all[l][c][1].append(g_all[l][c][2][t-1]*rfP*(1-rfQ1)) #to-do: essa equação só leva em conta 1 hospedeiro.
		if g_all[l][c][1][t]<=0.01:
			g_all[l][c][1][t]=0
...
def loops_cenarios(loop,viz,lista_patches,lista_borda):
	...
	###
	#testes:
	if loop==0: #1para especialista+1host
		replicas=1 #replicas
		TS=[4,0,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,0,0]#esse valor é o que a outra aluna da professora achou #esta com 0.1 pra nao dividir por zero
		...
		lambda2=0
		h0=[100,0.00001]#esta com 0.1 pra nao dividir por zero
		H0=[300,0.00001]#esta com 0.1 pra nao dividir por zero
		f0=[100,0.00001]#esta com 0.1 pra nao dividir por zero
		taxa_disp=[0.25,0,0.05,0]  #PQNH
		tempo_final=100
		inicial_n=100
		inicial_h=0
		#
		...
		para_lista_inicial=[6000,0]#é usado 6k por soltura com a cotesia
		
		
		lista_raios=viz[1] #salvando a lista de raios
		viz=viz[0] #salvando a lista de vizinhos
		indv_migrante=[[0.9,0.9,0.3,0]]#taxa de qnts parasitoides saem do patch baixa, média e alta.
		porcentagens=[0.15,1] #ocupacao inicial do host
		for solt_posi,solt_valor in enumerate(solturas):
			for pop_host_ini_porcentagem in porcentagens:
				...
				for fracao_indv_migrante in indv_migrante:
					if solt_valor==1:
						for ponto1 in solturas1ponto: 
							tempo=sorteio_pop_inicial_host(lin,col,porcentagem_popinihost,inicial_h,inicial_n)
							patch_iniciais=tempo[0]
							pop_iniciais=tempo[1]
							if ponto1 in patch_iniciais:
								for i,j in enumerate(patch_iniciais): #i posição, j valor
									if j==ponto1:
										pop_iniciais[i][0]=para_lista_inicial[0]
							else:
								patch_iniciais.append(ponto1)
								pop_iniciais.append([para_lista_inicial[0],para_lista_inicial[1],inicial_n,0])
							wd_novo=f'{wd_original}\\{solt_valor}\\{ponto1}\\{pop_host_ini_porcentagem}\\{fracao_indv_migrante}'
							os.makedirs(wd_novo)
							os.chdir(wd_novo)
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
					else:
						for ponto5 in solturas5pontos:	
							tempo=sorteio_pop_inicial_host(lin,col,porcentagem_popinihost,inicial_h,inicial_n)
							patch_iniciais=tempo[0]
							pop_iniciais=tempo[1]
							if ponto5[0] in patch_iniciais:
								for i,j in enumerate(patch_iniciais): #i posição, j valor
									if j==ponto5[0]:
										pop_iniciais[i][0]=para_lista_inicial[0]
							else:
								patch_iniciais.append(ponto5[0])
								pop_iniciais.append([para_lista_inicial[0],para_lista_inicial[1],inicial_n,0])

							if ponto5[1] in patch_iniciais:
								for i,j in enumerate(patch_iniciais): #i posição, j valor
									if j==ponto5[1]:
										pop_iniciais[i][0]=para_lista_inicial[0]
							else:
								patch_iniciais.append(ponto5[1])
								pop_iniciais.append([para_lista_inicial[0],para_lista_inicial[1],inicial_n,0])
							
							if ponto5[2] in patch_iniciais:
								for i,j in enumerate(patch_iniciais): #i posição, j valor
									if j==ponto5[2]:
										pop_iniciais[i][0]=para_lista_inicial[0]
							else:
								patch_iniciais.append(ponto5[2])
								pop_iniciais.append([para_lista_inicial[0],para_lista_inicial[1],inicial_n,0])
							
							if ponto5[3] in patch_iniciais:
								for i,j in enumerate(patch_iniciais): #i posição, j valor
									if j==ponto5[3]:
										pop_iniciais[i][0]=para_lista_inicial[0]
							else:
								patch_iniciais.append(ponto5[3])
								pop_iniciais.append([para_lista_inicial[0],para_lista_inicial[1],inicial_n,0])
							
							if ponto5[4] in patch_iniciais:
								for i,j in enumerate(patch_iniciais): #i posição, j valor
									if j==ponto5[4]:
										pop_iniciais[i][0]=para_lista_inicial[0]
							else:
								patch_iniciais.append(ponto5[4])
								pop_iniciais.append([para_lista_inicial[0],para_lista_inicial[1],inicial_n,0])
							
							wd_novo=f'{wd_original}\\{solt_valor}\\{ponto5}\\{pop_host_ini_porcentagem}\\{fracao_indv_migrante}'
							os.makedirs(wd_novo)
							os.chdir(wd_novo)
							for zzzz in range(replicas): 
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==1: #1 para generalista+1host
		replicas=1 #replicas
		TS=[0,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[0,1.46,0]#esse valor é o que a outra aluna da professora achou #esta com 0.1 pra nao dividir por zero
		a1=0 #esse valor é o que a outra aluna da professora achou
		a2=0.28
		a3=0
		lambda1=1.0341#esse é o valor tirado da tese da tabela de vida desse bixo
		lambda2=0
		h0=[0.00001,100]#esta com 0.1 pra nao dividir por zero
		H0=[0.00001,300]#esta com 0.1 pra nao dividir por zero
		f0=[0.00001,100]#esta com 0.1 pra nao dividir por zero
		taxa_disp=[0,0.25,0.05,0]  #PQNH
		tempo_final=100
		inicial_n=100
		inicial_h=0
		#
		solturas=[1,5]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[0,7000]#é usado 6k por soltura com a cotesia
					...
					if solt_valor==1:
						for ponto1 in solturas1ponto: 
							...
							if ponto1 in patch_iniciais:
								for i,j in enumerate(patch_iniciais): #i posição, j valor
									if j==ponto1:
										pop_iniciais[i][1]=para_lista_inicial[1]
							else:
								patch_iniciais.append(ponto1)
								pop_iniciais.append([para_lista_inicial[0],para_lista_inicial[1],inicial_n,0])
							wd_novo=f'{wd_original}\\{solt_valor}\\{ponto1}\\{pop_host_ini_porcentagem}\\{fracao_indv_migrante}'
						...
						for ponto5 in solturas5pontos:	
							...
							for zzzz in range(replicas): 
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	#simulações em si:
	elif loop==2:#2para+1host #juntos
		replicas=1 #replicas
		...
		H0=[300,300]
		f0=[100,100]
		taxa_disp=[0.25,0.25,0.05,0]  #PQNH
		tempo_final=100
		inicial_n=100
		inicial_h=0
		#
				...
				for fracao_indv_migrante in indv_migrante:
					if solt_valor==1:
						...
						for ponto5 in solturas5pontos:	
							...
							for zzzz in range(replicas): 
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==3: #2para+1host #separados 
		#replicas da simulação 2 parasitoide +1host (fração de indv migrantes com 0.1, 0.3 e 0.6)
		#
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		...
		solturas=[1,5]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[6000,7000]#é usado 6k por soltura com a cotesia e o generalista é 7k só placeholder
					...
					if solt_valor==1:
						for ponto1 in solturas1ponto: 
							...
							if ponto1 in patch_iniciais:
								...
								patch_iniciais.append(ponto1)
								pop_iniciais.append([para_lista_inicial[0],0,inicial_n,0])
								if ponto1==[0,25]:
									if [49,49] in patch_iniciais:
										for zzz,zzzz in patch_iniciais:
											if zzzz == [49,49]:
												pop_iniciais[zzz][1]=para_lista_inicial[1]
									else:
										patch_iniciais.append([49,49])
										pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								else:
									if [49,25] in patch_iniciais:
										for zzz,zzzz in patch_iniciais:
											if zzzz == [49,25]:
												pop_iniciais[zzz][1]=para_lista_inicial[1]
									else:
										patch_iniciais.append([49,25])
										pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
						...
						for ponto5 in solturas5pontos:	
							...
							#colocando a pop de q
							if ponto5[0]==[25, 5]:
								if [0,25] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[0,25]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([0,25])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								if [49,25] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[49,25]:
									...
									patch_iniciais.append([49,25])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								if [25, 25] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 25]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([25, 25])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								if [25, 0] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 0]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([25, 0])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								if [25, 49] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 49]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([25, 49])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
							else:
								if [25, 5] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 5]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([25, 5])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								if [25, 15] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 15]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([25, 15])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								if [25, 25] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 25]:
									...
									patch_iniciais.append([25, 25])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])								
								if [25, 35] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 35]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([25, 35])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])
								if [25, 45] in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==[25, 45]:
											pop_iniciais[i][1]=para_lista_inicial[1]
								else:
									patch_iniciais.append([25, 45])
									pop_iniciais.append([0,para_lista_inicial[1],inicial_n,0])		
							wd_novo=f'{wd_original}\\{solt_valor}\\{ponto5}\\{pop_host_ini_porcentagem}\\{fracao_indv_migrante}'
							os.makedirs(wd_novo)
							os.chdir(wd_novo)
							for zzzz in range(replicas): 
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==4:#2para+1host #juntos
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
				...
				for fracao_indv_migrante in indv_migrante:
					if solt_valor==1:
						...
						for ponto5 in solturas5pontos:	
							...
							for zzzz in range(replicas): 
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==5: #2para+1host #separados 
		#replicas da simulação 2 parasitoide +1host (fração de indv migrantes com 0.1, 0.3 e 0.6)
		#
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		...
		solturas=[1,5]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[100,200]#é usado 6k por soltura com a cotesia e o generalista é 7k só placeholder
					...
					if solt_valor==1:
						...
						for ponto5 in solturas5pontos:	
							...
							for zzzz in range(replicas): 
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	###  
	#com 10 hectares com [4,8] solturas por hectare e com pop inicial host de 100
	###
	elif loop==6: #só cotesia
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
		...
		solturas=[1,6,8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[6000,1500,1000],[0,0,0]]#[[6000,1500,1000],[7000,1750,1166.6666666666667]] #to-do: acomodar esse novo jeito nos outros loops e na função
		
		
		lista_raios=viz[1] #salvando a lista de raios
		viz=viz[0] #salvando a lista de vizinhos
		indv_migrante=[[0.9,0.9,0.3,0]]#[[0.9,0.9,0.3,0]]#taxa de qnts parasitoides saem do patch baixa e alta. 
		porcentagens=[0.15,1] #ocupacao inicial do host
		for solt_posi,solt_valor in enumerate(solturas):
			for pop_host_ini_porcentagem in porcentagens:
				porcentagem_popinihost=[pop_host_ini_porcentagem,0]
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						for patch_alternativo,hec10_patchinicial in enumerate(hec10_BordaMeio): #ta pegando só a lista de patch
							tempo=sorteio_pop_inicial_host(lin,col,porcentagem_popinihost,inicial_h,inicial_n)
							patch_iniciais=tempo[0]
							pop_iniciais=tempo[1]
							for patch_para in hec10_patchinicial:
								if patch_para in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==patch_para:
											pop_iniciais[i][0]=para_lista_inicial[0][solt_posi]
								else:
									patch_iniciais.append(patch_para)
									pop_iniciais.append([para_lista_inicial[0][solt_posi],0,inicial_n,0])
							...
							if borda_meio==0:
								...
								bOUm="meio"
							if patch_alternativo==0:
								p_a="patchesINI1"
							else:
								p_a="patchesINI2"
							wd_novo=f'{wd_original}\\{solt_valor}\\{pop_host_ini_porcentagem}\\{fracao_indv_migrante}\\{bOUm}\\{p_a}'
							os.makedirs(wd_novo)
							os.chdir(wd_novo)
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)			
	elif loop==7: #só t. howardi
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
		...
		solturas=[1,6,8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[0,0,0],[7000,1750,1166.6666666666667]]#[[6000,1500,1000],[7000,1750,1166.6666666666667]]  #to-do: acomodar esse novo jeito nos outros loops e na função
						...
						for patch_alternativo,hec10_patchinicial in enumerate(hec10_BordaMeio): #ta pegando só a lista de patch
							...
							for patch_para in hec10_patchinicial:
								if patch_para in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==patch_para:
											pop_iniciais[i][1]=para_lista_inicial[1][solt_posi]
								else:
									patch_iniciais.append(patch_para)
									pop_iniciais.append([0,para_lista_inicial[1][solt_posi],inicial_n,0])
							...
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==8: #ambos parasitoides juntos
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
		...
		solturas=[1,6,8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[6000,1500,1000],[7000,1750,1166.6666666666667]]  #to-do: acomodar esse novo jeito nos outros loops e na função
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						for patch_alternativo,hec10_patchinicial in enumerate(hec10_BordaMeio): #ta pegando só a lista de patch
							...
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==9: #ambos parasitoides separados
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						for patch_alternativo,hec10_patchinicial in enumerate(hec10_BordaMeio): #ta pegando só a lista de patch
							...
							for patch_para in hec10_BordaMeio[patch_alternativo-1]:
								if patch_para in patch_iniciais:
									for i,j in enumerate(patch_iniciais):
										if j==patch_para:
											pop_iniciais[i][1]=para_lista_inicial[1][solt_posi]
							...
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==10: #só Cotesia no dois pontos ao mesmo tempo
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
		...
		solturas=[1,6,8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[6000,1500,1000],[0,0,0]]#[7000,1750,1166.6666666666667]]  #to-do: acomodar esse novo jeito nos outros loops e na função
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						for patch_alternativo,hec10_patchinicial in enumerate(hec10_BordaMeio): #ta pegando só a lista de patch
							...
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==11: #só T. howardi no dois pontos ao mesmo tempo
		replicas=1 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
		...
		solturas=[1,6,8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[0,0,0],[7000,1750,1166.6666666666667]]  #to-do: acomodar esse novo jeito nos outros loops e na função
						...
						for patch_alternativo,hec10_patchinicial in enumerate(hec10_BordaMeio): #ta pegando só a lista de patch
							...
							for patch_para in hec10_BordaMeio[patch_alternativo-1]:
								if patch_para in patch_iniciais:
									...
									patch_iniciais.append(patch_para)
									pop_iniciais.append([0,para_lista_inicial[0][solt_posi],inicial_n,0])
							...
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	### 
	#com 10 hectares com [1,6,8] solturas por hectare e com pop inicial host de 25
	###
	elif loop==12: #só cotesia
		...
		solturas=[1,4,8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[6000,1500,1000],[0,0,0]]#to-do: acomodar esse novo jeito nos outros loops e na função
			...
			for pop_host_ini_porcentagem in porcentagens:
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						...
						patch_iniciais=tempo[0]
						pop_iniciais=tempo[1]
						for patch_para in hec10_BordaMeio[0]:
							if patch_para in patch_iniciais:
								for i,j in enumerate(patch_iniciais):
									if j==patch_para:
										pop_iniciais[i][0]=para_lista_inicial[0][solt_posi]
						...
						if borda_meio==0:
							...
							bOUm="meio"
						wd_novo=f'{wd_original}\\{solt_valor}\\{pop_host_ini_porcentagem}\\{bOUm}'
						os.makedirs(wd_novo)
						os.chdir(wd_novo)
						for zzzz in range(replicas):
							modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)			
	elif loop==13: #só t. howardi
		replicas=10 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
			...
			for pop_host_ini_porcentagem in porcentagens:
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						...
						patch_iniciais=tempo[0]
						pop_iniciais=tempo[1]
						for patch_para in hec10_BordaMeio[1]:
							if patch_para in patch_iniciais:
								for i,j in enumerate(patch_iniciais):
									if j==patch_para:
										pop_iniciais[i][1]=para_lista_inicial[1][solt_posi]
						...
						for zzzz in range(replicas):
							modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==14: #ambos parasitoides juntos
		replicas=10 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
			...
			for pop_host_ini_porcentagem in porcentagens:
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						...
						for zzzz in range(replicas):
							modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==15: #ambos parasitoides separados
		replicas=10 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						#for patch_alternativo,hec10_patchinicial in enumerate(hec10_BordaMeio): #ta pegando só a lista de patch
							...
							for zzzz in range(replicas):
								modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==16: #só Cotesia no dois pontos ao mesmo tempo
		replicas=10 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
			...
			for pop_host_ini_porcentagem in porcentagens:
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						...
						for zzzz in range(replicas):
							modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz)
	elif loop==17: #só T. howardi no dois pontos ao mesmo tempo
		replicas=10 #replicas
		TS=[4,4,0] #esse valor é o que a outra aluna da professora achou
		TH=[1.46,3.07,0]#esse valor é o que a outra aluna da professora achou
		a1=0.28 #esse valor é o que a outra aluna da professora achou
		...
		solturas=[1]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[0,0,0],[0,0,0]]  #to-do: acomodar esse novo jeito nos outros loops e na função
	...
	##############################
		...
		solturas=[8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[1000],[0]]#to-do: acomodar esse novo jeito nos outros loops e na função
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						...
						while replicas<10 or total_replicas<50:
							modelo_paisagem_migracao(a1,a2,a3,lambda1,lambda2,H0,h0,f0,taxa_disp,fracao_indv_migrante,tempo_final,lin,col,pop_iniciais,patch_iniciais,viz,lista_raios,TS,TH,zzzz,patches_sorteados,bOUm,solt_posi,solt_valor,hec10_bordas,para_lista_inicial,borda_meio,loop,lib_extras)
							...
							if ocupacao[0][10]>0:
								replicas+=1		
	elif loop==20: #ambos parasitoides juntos
		...
		solturas=[8]
		wd_original=f'{os.getcwd()}'
		para_lista_inicial=[[1000],[1166.6666666666667]]#to-do: acomodar esse novo jeito nos outros loops e na função
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						...
						while replicas<10 or total_replicas<50:
							...
							if ocupacao[0][10]>0:
								replicas+=1
	elif loop==21: #ambos parasitoides separados
				...
				for fracao_indv_migrante in indv_migrante:
					for borda_meio,hec10_BordaMeio in enumerate(hec10[solt_posi]): #pega borda e meio
						...
						while replicas<10 or total_replicas<50:
							...
							ocupacao=pck.loads(ocupacao_pck)
							total_replicas+=1
							if ocupacao[0][10]>0 and ocupacao[1][10]>0:
								replicas+=1
	elif loop==22: #só Cotesia no dois pontos ao mesmo tempo
...
lib_extras=str(input("Digite False para apenas a liberação inicial e True para permitir liberação nas outras gerações: "))
if lib_extras=="False" or lib_extras=="F" or lib_extras=="false" or lib_extras=="f" or lib_extras=="Fasle" or lib_extras=="fasle":
	print("deu que é falso a liberacao extra")
	lib_extras=False
elif lib_extras=="True" or lib_extras=="true" or lib_extras=="T" or lib_extras=="t":
	print("deu que é verdadeiro liberacao extras")
	lib_extras=True
else:
	print("deu ruim em definir se quer lib extra ou nao")


loop=int(input("5 hectares:\n     0- 1para especialista + 1host\n     1- 1para generalista + 1host\n     2- 2paraJuntos + 1host\n     3- 2paraSeparados + 1host\n     4- 2paraJuntos POP baixa + host\n     5- 2paraSEPARADOS POP baixa + host \n10 hectares:\n     6- Apenas Cotesia\n     7- Apenas T. howardi\n     8- 2paraJuntos + 1host\n     9- 2paraSeparados + 1host\n     10- Cotesia ambos patches + 1host\n     11- T. howardi ambos patches + 1host\n10 hectares e população de host baixa:\n     12- Apenas Cotesia\n     13- Apenas T. howardi\n     14- 2paraJuntos + 1host\n     15- 2paraSeparados + 1host\n     16- Cotesia ambos patches + 1host\n     17- T. howardi ambos patches + 1host\n     18- só praga\n10 hectares e população de host baixa sem q os patches iniciais de parasitoide aumentem ocupacao do host, borda x meio sorteado, tempo_final=10, liberacao de parasitoide:\n     19- Apenas Cotesia\n     20- 2paraJuntos + 1host\n     21- 2paraSeparados + 1host\n     22- Cotesia ambos patches + 1host\n\nPS: Ao escolher o 12 ele irá rodar os cenários 12 ao 17 e se escolher 19 irá rodar do 19 ao 22.\nEscolha qual simulação: "))
lin=50
col=50
distancia_maxima=3
...
if loop==12:
	...
	for loop_numero in [loop,loop+1,loop+2,loop+3,loop+4,loop+5]:
		if loop_numero==12:
			...
			os.chdir(pasta)
		if loop_numero==13:
			pasta=f'{wd_pasta_origem}\\so howardi'
			os.makedirs(pasta)
			os.chdir(pasta)
		if loop_numero==14:
		...
		if loop_numero==15:
			...
			os.chdir(pasta)
		if loop_numero==16:
			pasta=f'{wd_pasta_origem}\\cotesia nos dois'
			os.makedirs(pasta)
			os.chdir(pasta)
		if loop_numero==17:
			pasta=f'{wd_pasta_origem}\\howardi nos dois'
			os.makedirs(pasta)
			os.chdir(pasta)
		loops_cenarios(loop_numero,viz,lista_patches,lista_borda)
elif loop==19:
	wd_pasta_origem=f'{os.getcwd()}'
	for loop_numero in [loop,loop+1,loop+2,loop+3]:
		if loop_numero==19:
			pasta=f'{wd_pasta_origem}\\so cotesia'
			os.makedirs(pasta)
			os.chdir(pasta)
	...
	loops_cenarios(loop,viz,lista_patches,lista_borda)

	



#to-do: o modelo parace certo, só falta as situações pra qual vou correr e os graficos.
#to-do: qnd os dois parasitoides sao soltos juntos, o generalista nao consegue sobreviver pois o especialista é um competitor muito bom
#mas se eles forem soltos em pontos diferentes, o generalista cresce bastante (qnd n tem competição).
#nao testei qnd tem pouco especialista em um ponto e bastante generalista pra ver oq rola.
#to-do: deletar o loop==2 pq ele é só um teste.